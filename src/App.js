...
export default function AtlasFitnessApp() {
  ...
  const detectEquipmentFromImage = async (imageBase64) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Equipment detection failed');
      }

      const data = await response.json();
      setSelectedEquipment(data.equipment || ['dumbbells', 'bodyweight exercises']);
      setStep('equipment');
    } catch (err) {
      setError('Detection failed. You can add equipment manually.');
  ...
  const shareWorkout = async () => {
    const shareText = `Check out my ${difficulty} ${goal} workout! Generated by Atlas Fitness Coach.`;
    try {
      if (navigator.share) {
    ...
    <div style={{ minHeight: '100vh', background: '#0a0e27', fontFamily: '"Inter", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        h2 { font-size: 24px; font-weight: 700; margin-bottom: 1.5rem; }
        h3 { font-size: 18px; font-weight: 600; margin-bottom: 1rem; }
          ...
          <div className="content">
            ...
            {step === 'equipment' && (
              <div>
                ...
                {uploadedImage && !detectionLoading && <img src={uploadedImage} alt="Gym setup" style={{ width: '100%', maxWidth: '400px', borderRadius: '16px', marginBottom: '1.5rem' }} />}
                <div style={{ marginBottom: '1.5rem' }}>
                  <p style={{ fontSize: 14, fontWeight: 500, marginBottom: '1rem' }}>Equipment:</p>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem' }}>
                    {selectedEquipment.map(item => <span key={item} className="badge badge-info">{item}</span>)}
                  </div>
                </div>
                <div style={{ marginBottom: '1.5rem' }}>
                  <p style={{ fontSize: 14, fontWeight: 500, marginBottom: '1rem' }}>Add Equipment:</p>
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <input type="text" placeholder="e.g., water bottles, rope" value={manualEquipment} onChange={(e) => setManualEquipment(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && addManualEquipment()} />
                    <button className="button" onClick={addManualEquipment} style={{ padding: '0.75rem 1.5rem', width: 'auto' }}>Add</button>
                  </div>
                </div>
                ...
                <div className="grid-2">
                  {['beginner', 'intermediate', 'advanced'].map(d => (
                    <button key={d} onClick={() => { setDifficulty(d); setStep('time'); }} className={`card ${difficulty === d ? 'selected' : ''}`} style={{ textAlign: 'center', padding: '1.5rem' }}>
                      <div style={{ fontSize: 16, fontWeight: 600, textTransform: 'capitalize' }}>{d}</div>
                      <div style={{ fontSize: 12, color: '#9ca3af', marginTop: '0.5rem' }}>
                        {d === 'beginner' && 'Foundation'}
                        {d === 'intermediate' && 'Balanced'}
                        {d === 'advanced' && 'Intense'}
                      </div>
                    </button>
                  ))}
            ...
            {step === 'limits' && (
              <div>
                ...
                <textarea placeholder="e.g., weak back, no jumping..." value={limitations} onChange={(e) => setLimitations(e.target.value)} style={{ marginBottom: '1.5rem', minHeight: '100px' }} />
            ...
            {step === 'results' && generatedWorkout && !selectedExercise && (
              <div>
                <div style={{ background: 'linear-gradient(135deg, rgba(59, 130, 246, 0.15), rgba(139, 92, 246, 0.1))', border: '1px solid rgba(59, 130, 246, 0.3)', borderRadius: '16px', padding: '2rem', marginBottom: '2rem', textAlign: 'center' }}>
                  <p style={{ fontSize: 20, fontWeight: 600, margin: '0 0 1rem', fontStyle: 'italic' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
                    ...
                    <span className="badge badge-success">{timeLimit}m</span>
                  </div>
                </div>
                ...
                {generatedWorkout.mainCircuit && generatedWorkout.mainCircuit.map((ex, idx) => (
                  <button key={idx} onClick={() => setSelectedExercise(ex)} className="exercise-card">
                    <p style={{ fontSize: 16, fontWeight: 600, margin: '0 0 0.5rem' }}>{ex.exercise}</p>
                    <p style={{ fontSize: 13, color: '#9ca3af', margin: '0 0 0.25rem' }}>{ex.description}</p>
                    <p style={{ fontSize: 12, color: '#6b7280', margin: 0 }}>{ex.duration}</p>
                  </button>
                ))}

                <h3 style={{ marginTop: '2rem' }}>MacGyver Move</h3>
                <div style={{ background: 'linear-gradient(135deg, rgba(16, 185, 129, 0.15), rgba(5, 150, 105, 0.1))', border: '1px solid rgba(16, 185, 129, 0.3)', borderRadius: '16px', padding: '1.5rem', marginBottom: '2rem' }}>
                  <p style={{ fontSize: 16, fontWeight: 600, margin: '0 0 0.5rem', color: '#10b981' }}>{generatedWorkout.macgyverMove && generatedWorkout.macgyverMove.name}</p>
                  <p style={{ fontSize: 14, color: '#6ee7b7', margin: 0 }}>{generatedWorkout.macgyverMove && generatedWorkout.macgyverMove.description}</p>
                </div>
                ...
                <h3 style={{ marginTop: '2rem' }}>Cool Down</h3>
                <p style={{ marginBottom: '2rem' }}>{generatedWorkout.cooldown}</p>

                <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                  <button className="button button-primary" onClick={shareWorkout} style={{ flex: 1 }}>
                    Share
                  </button>
            ...
            {selectedExercise && (
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ background: 'rgba(59, 130, 246, 0.1)', border: '1px solid rgba(59, 130, 246, 0.2)', borderRadius: '16px', padding: '1.5rem', marginBottom: '1.5rem' }}>
                  <h3 style={{ marginTop: 0 }}>{difficulty}</h3>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginTop: '1rem' }}>
                    <div>
                      <p style={{ fontSize: 12, color: '#9ca3af', margin: '0 0 0.5rem' }}>Reps</p>
                      <p style={{ fontSize: 18, fontWeight: 700, margin: 0, color: '#60a5fa' }}>{EXERCISE_DATABASE[selectedExercise.exercise]?.[difficulty]?.reps}</p>
                    </div>
                    <div>
                      <p style={{ fontSize: 12, color: '#9ca3af', margin: '0 0 0.5rem' }}>Sets</p>
                      <p style={{ fontSize: 18, fontWeight: 700, margin: 0, color: '#60a5fa' }}>{EXERCISE_DATABASE[selectedExercise.exercise]?.[difficulty]?.sets}</p>
                    </div>
                    <div>
                      <p style={{ fontSize: 12, color: '#9ca3af', margin: '0 0 0.5rem' }}>Rest</p>
                      <p style={{ fontSize: 18, fontWeight: 700, margin: 0, color: '#60a5fa' }}>{EXERCISE_DATABASE[selectedExercise.exercise]?.[difficulty]?.rest}</p>
                    </div>
                  </div>
                </div>

                <div style={{ background: 'rgba(245, 158, 11, 0.15)', border: '1px solid rgba(245, 158, 11, 0.3)', borderRadius: '16px', padding: '1.5rem', marginBottom: '1.5rem', textAlign: 'left' }}>
